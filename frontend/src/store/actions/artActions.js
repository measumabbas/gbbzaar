...
export const uploadArt = (artData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      "http://localhost:4000/api/v1/book/create",
      artData
    );
...
export const getAllArts =
  (language, availability, condition, title) => async (dispatch) => {
    try {
      dispatch({ type: GET_ALL_ARTS_REQUEST });
      let url = "http://localhost:4000/api/v1/books?category=all";
...
export const getUserArts = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post("http://localhost:4000/api/v1/book/me", {
      id,
    });
...
export const deleteUserArt = (id) => async (dispatch) => {
  try {
    ...
    await axios.delete(`http://localhost:4000/api/v1/book/${id}`);
...
export const getSingleArt = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:4000/api/v1/book/${id}`);
...
export const addArtReview = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `http://localhost:4000/api/v1/book/reviews/add`,
      userData
    );
...
export const getArtReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `http://localhost:4000/api/v1/book/reviews/get?id=${id}`
    );
    dispatch({ type: GET_ART_REVIEWS_SUCCESS, payload: data.reviews });
  } catch (error) {
    console.log(error);
    dispatch({
      type: GET_ART_REVIEWS_FAIL,
...
export const updateArt = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `http://localhost:4000/api/v1/book/${userData.artId}`,
      userData
    );
